...
def chunk_text(text, max_length=512, overlap=50):
    """
    Splits text into chunks of max_length tokens with optional overlap.
    Ensures no text is left out.
    """
    import re
...
def process_large_doc(nlp, text, max_length=512, overlap=50):
    """
    Processes large text by chunking and merging results.
    """
    logger = logging.getLogger("postprocessing")
    logger.debug("Chunking text for large document processing.")
    ...
    for i, chunk in enumerate(chunks):
        ...
        doc = nlp(chunk)
        docs.append(doc)
    # Merge docs into one large doc
    from spacy.tokens import Doc
    logger.debug("Merging processed chunks into a single Doc.")
...
def postprocessing(nlp_doc):
    ...
    try:
        ...
        try:
            ...
            raise
        # Other person coreference resolution
        try:
            ...
            print(traceback.format_exc())
            raise
        # Provision-statute coreference resolution
        try:
